...
- Setup `Pre Issue Access Token` and `Pre Issue ID Token` actions with the iam-service-extensions Ballerina service path.
    ```text
    Pre Issue Access Token - https://<pre-issue-extension-base-path>:/pre-issue-access-token
    Pre Issue ID Token - https://<pre-issue-extension-base-path>:/pre-issue-id-token
    ```
